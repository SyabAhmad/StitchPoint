...
export default function NewArrivals() {
  ...
  useEffect(() => {
    const fetchNewArrivals = async () => {
      try {
        const response = await fetch("http://127.0.0.1:5000/api/products/new-arrivals");
        ...
        {products.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {products.slice(0, 6).map((product) => (
              <div
                key={product.id}
                className="group bg-white rounded-xl overflow-hidden border border-black/5 hover:shadow-md transition-all duration-300 cursor-pointer"
              ...
              >
                <div className="relative aspect-[4/5] overflow-hidden bg-black/[0.03]">
                  ...
                  {product.sale_type && product.sale_discount_percentage > 0 && (
                    <span className="absolute top-2.5 left-2.5 bg-red-500 text-white text-[10px] font-semibold px-2 py-0.5 rounded">
                ...
                <div className="p-3.5">
                  ...
                  <div className="flex items-center gap-2">
                    {product.sale_type && product.sale_discount_percentage > 0 ? (
                      ...
                      <span className="text-black font-bold text-sm">PKR {product.price.toLocaleString()}</span>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="text-center py-12">
            <p className="text-black/25 text-sm">No new arrivals yet. Check back soon.</p>
          </div>
        )}
      </div>
    </section>
